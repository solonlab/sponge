...
<html class="frm10">
<head>
    <title>${app} - <c:choose><c:when test="${is_edit}">编辑动作</c:when><c:otherwise>新建动作</c:otherwise></c:choose></title>
    ...
    <script>
        $(function () {
            ...
            $("#name_display").val("${action.name_display}");
            </c:if>
        });
        ...
        function save() {
            ...
            if(!name){
                top.layer.msg("动作代号不能为空");
                return;
            }
            if(!name_display){
                top.layer.msg("动作名称不能为空");
                return;
            }
            $.ajax({
                type: "POST",
                url: "/push/action/ajax/save",
                data: {
                    <c:if test="${is_edit}">action_id: ${action.action_id}, </c:if>
                    agroup_id: ${agroup_id},
                    name: name,
                ...
                success: function (data) {
                    if (data.code === 1) {
                        ...
                        location.href = "/push/action/inner/" + ${agroup_id};
                    } else {
                        top.layer.msg(data.msg);
                    }
                }
        ...
        function del() {
            top.layer.confirm('确定删除动作', {
                btn: ['确定','取消'] //按钮
            }, function(){
                $.ajax({
                    type: "POST",
                    url: "/push/action/ajax/del",
                    data: {
...
<main>
    <detail><form>
        <h2><c:choose><c:when test="${is_edit}">编辑动作</c:when><c:otherwise>新建动作</c:otherwise></c:choose>（<a class="t2" onclick="history.back(-1)">返回</a>）</h2>
        <hr/>
        <table>
            <tr>
                <td>动作名称</td>
                <td>
                    <input type="text" placeholder="输入动作名称" id="name_display"/>
                </td>
            </tr>
            <tr>
                <td>动作代号</td>
            ...
            <tr>
                <td>动作代码</td>
                <td><p><textarea id="code"><c:if test="${is_edit}">${action.code}</c:if></textarea></p></td>
            </tr>
            <tr>
                <td></td>
                <td>
