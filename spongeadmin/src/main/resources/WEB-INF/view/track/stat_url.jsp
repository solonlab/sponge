...
<head>
    ...
    <script>
        ...
        function state_select(val,e) {
            getCharts(val,${url_id});
        };

        function state_select1(val,e) {
            getMapCharts(val,${url_id});
            stateVal = val;
        };

        function state_select2(val,e) {
            getPlatformCharts(val,0,${url_id});
            stateVal = val;
        };

        function state_select3(val,e) {
            getClientCharts(val,0,${url_id});
            stateVal = val;
        };
    </script>
    <style>
...
<main class="sml">
    ...
    <block>
        <div style="padding: 5px;overflow: hidden;">
            系统：<ct:stateselector items="PV,UV,IP" clientID="stat2" onSelect="state_select2"/>
    ...
    <block>
        <div style="padding: 5px;overflow: hidden;">
            终端：<ct:stateselector items="PV,UV,IP" clientID="stat3" onSelect="state_select3"/>
            <a style="float: right;cursor: pointer;" id="hidden4" onclick="isShow4();">收起</a>
        </div>
        <div id="client_charts" style="width:calc(100% - 13px);height: 250px;margin: auto;;cursor: default;"></div>
    </block>

    <block>
        <div style="padding: 5px;overflow: hidden;">
            区域：<ct:stateselector items="PV,UV,IP" clientID="stat1" onSelect="state_select1"/>
...
<script>
    function list_state_select(val,e) {
        $.get('/track/stat/url/inner?url_id=${url_id}&_state='+val,function (rst) {
            $('datagrid').html(rst);
        })
    }
    ...
    $(function () {
        get30Charts(${url_id});
        getCharts(0,${url_id});
        getMapCharts(0,${url_id})
        list_state_select(0,null);
        getPlatformCharts(0,0,${url_id});
        getClientCharts(0,0,${url_id});
    });
    ...
    function getCharts(type,url_id) {
        var hoursList=[];
        var todays=[];
        var yesterdays=[];
        ...
        $.ajax({
            ...
            data:{"id":url_id,"type":type,"queryType":1},
            success:function(data){
    ...
    function getMapCharts(type,url_id) {
        var yesterdays;
        var max;
        $.ajax({
    ...
    function getPlatformCharts(type,tag_id,url_id) {
        var yesterdays;
        var max;
        $.ajax({
            url:"/track/stat/ajax/getPlatformCharts",
            data:{"tag_id":tag_id,"type":type,"url_id":url_id},
            success:function(data) {
    ...
    function getClientCharts(type,tag_id,url_id) {
        $.ajax({
